...
from .CMU2D import CMU2D
from .G2D import G2D
class G_CMU2D(JointsDataset):
    """docstring for TencentHand"""
    def __init__(self, cfg):
        super(G_CMU2D, self).__init__(cfg)
        self.cmu = CMU2D(cfg.CMU)
        self.g = G2D(cfg.TENCENT)

    def __len__(self):
        # return 100
        return len(self.cmu) + len(self.g)
